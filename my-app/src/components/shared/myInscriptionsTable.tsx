...
export default function MyInscriptions() {
  ...
  const handleDeleteInscription = async (id: number) => {
    if (session?.token) {
      try {
        await deleteInscription(session.token, String(id), toast);
        setInscriptions(prevData => prevData.filter(item => item.id !== id));
      } catch (error) {
        console.log("Error ao tentar deletar inscrição: ", error);
      }
    }
  };
      ...
      <div className="p-5 h-full flex flex-col justify-between">
        <div>
          ...
          <div className="flex justify-between mb-4">
            <div><FilterInscriptions onFilterChange={setStatusFilter} /></div>
            <div><SearchComponent onSearch={setNameFilter} /></div>
          </div>
